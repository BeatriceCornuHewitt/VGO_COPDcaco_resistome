...
microbiome.ps@sam_data$SampleID
...
saveRDS(microbiome.ps, file = "..//Data//raw//ps_COPD_d2_f_01%in2samples_08APR2021.rds")
...
## Microbiome 
set.seed(123)
microbiome.PCoA <- ordinate( microbiome.ps.genus.hell, 
                             method="PCoA", 
                             distance="bray")

microbiome.PCoA.plot <- plot_ordination(microbiome.ps.genus.hell, microbiome.PCoA,
                                        color="copdcaco", 
                                        shape="copdcaco" ) +
...
set.seed(123)
protest(microbiome.procrustes.PCoA.data, resistome.procrustes.PCoA.data, scores = "sites", permutations = 9999) # correlation coefficient = 0.4643 , p= 1e-04, m-squared = 0.7844
...
set.seed(123)
protest(microbiome.PCoA.random, resistome.procrustes.PCoA.data, scores = "sites", permutations = 9999) # correlation coefficient = 0.08359, p= 0.8483 ,m-squared = 0.993 
...
PCoA.pro.plot <- ggplot(PCoA.pro.df) +
  ...
  geom_point(aes(x=xrda1, y=xrda2, colour=copdcontrol, shape="Resistome", fill=copdcontrol), size = 2) +
  geom_segment(aes(x=rda1, y=rda2, xend=xrda1, yend=xrda2, colour=copdcontrol), arrow=arrow(length=unit(0.2, "cm")))

Final.PCoa.pro <- PCoA.pro.plot + 
  ...
  labs(title = "PCoA procrustes", y = "PCo2", x = "PCo1", subtitle = "Correlation: 0.35, Significance: 6e-04", size = 16 ) +
  guides(fill = guide_legend(override.aes = list(shape = NA)))+ # Remove the contents of the legend
           theme(axis.text.x = element_text(size = 16), 
                 axis.text.y = element_text(size = 16), 
                 axis.title.x = element_text(size = 16),
                 axis.title.y = element_text(size = 16),
                 plot.subtitle = element_text(size=16))  # Adjust the size of x- and y-axis labels

ggsave(filename = "Output_files//Procrustes//Final.PCoa.pro.png", plot = Final.PCoa.pro, width = 8, height = 6, dpi = 1000)



Final.PCoa.pro2 <- PCoA.pro.plot + 
  scale_colour_manual(values = c("cornflowerblue", "orange")) +
  scale_shape_manual(values = c("Microbiome" = 21, "Resistome" = 24)) +
  scale_fill_manual(values = c("Control" = "cornflowerblue", "COPD" = "orange")) +
  labs(title = "PCoA procrustes", y = "PCo2", x = "PCo1", size = 16 ) +
  ...
  theme(
    ...
    axis.title.x = element_text(size = 16),
    axis.title.y = element_text(size = 16),
    plot.subtitle = element_text(size=16, hjust = 1, vjust = 0),
    plot.margin = margin(0, 0, 1, 1, "cm")  # Add a margin to make space for the subtitle
