...
Shannon.diversity.plot.pval <- Shannon.diversity.plot +
  stat_compare_means(method = "t.test", size = 5, label.x = 0.6, label.y = 3.4)
Simpson.diversity.plot.pval <- Simpson.diversity.plot +  stat_compare_means(method = "wilcox.test", size=5, label.x = 0.6, label.y = 0.42)
Observed.diversity.plot.pval <- Observed.diversity.plot + stat_compare_means(method = "wilcox.test", size=5,label.x = 0.6, label.y = 100)
...
# Settings for plot for manuscript
suppressWarnings(library(cowplot))
theme_set(theme_cowplot())
...
staphLUR_simpson_plot <- ggplot(LUR_RF_exposure_alphadiversity_df, aes(x = Staph_LUR_estimates, y = Simpson_Evenness)) +
  ...
  geom_text(x = max(LUR_RF_exposure_alphadiversity_df$Staph_LUR_estimates), y = max(LUR_RF_exposure_alphadiversity_df$Simpson_Evenness) - 0.06,
...
mecA_simpson_plot <- ggplot(LUR_RF_exposure_alphadiversity_df, aes(x = mecA_RF_predictions, y = Simpson_Evenness)) +
  ...
  geom_abline(intercept = coef(lm_simpeveness_mecA_RF)[1], slope = coef(lm_simpeveness_mecA_RF)[2],
              color = "cornflowerblue", linetype = "solid", linewidth = 1.5) +
  # Add slope coefficient and p-value as annotations
  ...
  geom_text(x = max(LUR_RF_exposure_alphadiversity_df$mecA_RF_predictions), y = max(LUR_RF_exposure_alphadiversity_df$Simpson_Evenness) - 0.06,
...
mecA_simpson_plot <- ggplot(LUR_RF_exposure_alphadiversity_df, aes(x = mecA_RF_predictions, y = Simpson_Evenness)) +
  # Customize the scatterplot points
  geom_point(size = 4, color = "#0072B2", alpha = 0.7) +
  # Add a regression line
  geom_smooth(method = "lm", se = FALSE, color = "#D55E00", linetype = "dashed", linewidth = 1.5) +
  # Add slope coefficient and p-value as annotations
  geom_text(x = max(LUR_RF_exposure_alphadiversity_df$mecA_RF_predictions), y = max(LUR_RF_exposure_alphadiversity_df$Simpson_Evenness) - 0.03,
            label = paste("Slope coefficient =", round(slope_coef, 3)), color = "#D55E00", size = 6, hjust = 1, vjust = 1) +
  geom_text(x = max(LUR_RF_exposure_alphadiversity_df$mecA_RF_predictions), y = max(LUR_RF_exposure_alphadiversity_df$Simpson_Evenness) - 0.06,
            label = paste("p-value =", signif(p_value, digits = 3)), color = "#D55E00", size = 6, hjust = 1, vjust = 1) +
  # Customize x and y axis labels and title
  labs(x = "Residential exposure to mecA (RF-modelled)\n(gene copies/m^2)", y = "Simpson's Evenness index",
       title = "Simpson's Evenness index vs. mecA exposure estimates") +
  # Customize the theme (adjust the appearance of the plot)
  theme_minimal() +
...
Simpson.diversity.plot <- ggplot(metadata.final.ps.billion.noblanks, aes(x = copdcaco, y = SimpsonEvenness)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Study population") +
  ylab("Simpson Evenness diversity \nindex") +
  geom_point(aes(color = factor(copdcaco)), position = "jitter", size = 4, alpha = 0.3) +
  scale_fill_manual(values = c("cornflowerblue", "orange")) +
  scale_color_manual(values = c("cornflowerblue", "orange"))
