...
# Blanks 
blank_ps <- subset_samples(ARGcluster.aggl, copdcaco == "blanc")
# Access the otu_table and tax_table from the COPD_ps phyloseq object
...
Final.ps.top10ARGClusters <- prune_taxa(top10.ARGclusters.overall.noblanks, ARGcluster.aggl)

# Subset the phyloseq object to include only COPD and control samples
Final.ps.top10ARGClusters_noblanks <- subset_samples(Final.ps.top10ARGClusters, copdcaco %in% c("1", "0"))

# % of total taxa 
total_sum <- sum(taxa_sums(ARGcluster.aggl.noblanks))
top10_sum <- sum(taxa_sums(Final.ps.top10ARGClusters_noblanks))
...
Final.ps.top10ARGClusters_noblanks_COPD <- subset_samples(Final.ps.top10ARGClusters_noblanks, copdcaco %in% "1")
top10_sum_COPD <- sum(taxa_sums(Final.ps.top10ARGClusters_noblanks_COPD))
...
Final.ps.top10ARGClusters_noblanks_control <- subset_samples(Final.ps.top10ARGClusters_noblanks, copdcaco %in% "0")
top10_sum_control <- sum(taxa_sums(Final.ps.top10ARGClusters_noblanks_control))
...
# total_sum_copd <- sum(taxa_sums(subset_taxa(ARGcluster.aggl_subset, ARGcluster.aggl_subset@sam_data$copdcaco == "COPD")))
