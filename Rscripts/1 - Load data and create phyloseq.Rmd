...
# Select exposure columns (numeric only)
# Just choose the columns we want
expo_cols <- names(livestock_models)[3:10] # only keep columns 3-10

# Compute summary statistics (column-wise) safely
mean_values <- sapply(livestock_models[expo_cols], mean, na.rm = TRUE)
sd_values   <- sapply(livestock_models[expo_cols], sd, na.rm = TRUE)
median_vals <- sapply(livestock_models[expo_cols], median, na.rm = TRUE)
p25_values  <- sapply(livestock_models[expo_cols], quantile, probs = 0.25, na.rm = TRUE)
p75_values  <- sapply(livestock_models[expo_cols], quantile, probs = 0.75, na.rm = TRUE)
min_values  <- sapply(livestock_models[expo_cols], min, na.rm = TRUE)
max_values  <- sapply(livestock_models[expo_cols], max, na.rm = TRUE)
...
summary_statistics_df <- data.frame(
  ...
  SD = sd_values,
  Median = median_vals,
  `25th Percentile` = p25_values,
  `75th Percentile` = p75_values,
  Minimum = min_values,
  Maximum = max_values,
  row.names = NULL,
  check.names = FALSE
...
merged_exp <- merged_exp %>%
  ...
  mutate(educationlevel= recode(educationlevel, `0` = "Low", `1` = "Medium", `2` = "High"))%>%
