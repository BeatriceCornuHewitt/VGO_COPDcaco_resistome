...
phyl_otu = as(otu_table(Final.ps.DAanalysis.filtered.noblanks),"matrix") %>% as.data.frame()   # check otu file
phyl_tax = as(tax_table(Final.ps.DAanalysis.filtered.noblanks),"matrix") %>% as.data.frame()   # check tax file 
phyl_met = as(sample_data(Final.ps.DAanalysis.filtered.noblanks),"matrix") %>% as.data.frame() # check map file
...
# lfc_(Intercept) and lfc_copdcaco1: lfc = log fold changes in microbial abundance. lfc_(Intercept): This column represents the LFC for each taxon or feature between the reference group (Intercept) and itself. In other words, it shows the estimated change in abundance or expression of each taxon within the reference group. The LFC_(Intercept) values indicate how much the abundance or expression of each taxon varies within the reference group. fc_copdcaco1: This column represents the LFC for each taxon or feature between the reference group (Intercept) and the "copdcaco1" group. It shows the estimated change in abundance or expression of each taxon when comparing the reference group to the "copdcaco1" group. A positive LFC indicates an increase in abundance or expression in the "copdcaco1" group compared to the reference group, while a negative LFC suggests a decrease. The magnitude of the LFC_(copdcaco1) values provides information about the size of the difference between these two groups.
...
# Comparing absolute abundances - Wilcoxon rank sum test 
```{r}
