...
# Combined
# Sample number vs AMR class (COPD, controls & blanks)
AMRclass.bars.absolute <- plot_bar(AMRclass.aggl, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16S qPCR corrected resistome counts", x="Sample", y="Abundance") +
    theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) ) 
    # scale_y_continuous(limits = c(0,580), expand = c(0, 0))
AMRclass.bars.absolute
ggsave("Output_files//AMR_abundance_plots//Absolute_abundances//AMRclass.bars.absolute.png",width = 8, height = 4,dpi = 3000)

# Sample number vs AMR class (COPD & controls) 
AMRclass.bars.absolute.noblanks <- plot_bar( AMRclass.aggl.noblanks, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16S qPCR corrected resistome counts", x="Sample", y="Abundance") +
...
AMRclass.bars.absolute.noblanks
ggsave("Output_files//AMR_abundance_plots//Absolute_abundances//AMRclass.bars.absolute.noblanks.png",width = 8, height = 4,dpi = 3000)
...
# Define a common y-axis label
common_y_axis_label <- "Abundance"

# Create individual plots
plot_copd <- AMRclass.absolute.bars.copd +
  labs(y = common_y_axis_label) +
  geom_bar(stat = "identity", width = 1)  # Set the width for this plot

plot_control <- AMRclass.absolute.bars.control +
  labs(y = NULL) +
  geom_bar(stat = "identity", width = 0.97)  # Set the width for this plot

plot_blanks <- AMRclass.absolute.bars.blanks +
  labs(y = NULL) +
  geom_bar(stat = "identity", width = 0.09)  # Set the width for this plot

# Combine the plots using patchwork
combined_plots_absolute <- plot_copd | plot_control | plot_blanks

# Add a common title for the entire layout
COPDcontrolblank_plots_absolute <- combined_plots_absolute +
  plot_layout(ncol = 3, byrow = TRUE, guides = "collect") +
  plot_annotation(title = "Total rarefied, gene length corrected and 16S qPCR corrected resistome absolute abundance", theme = theme(plot.title = element_text(hjust = 0.5, size = 20)))  # Add a closing parenthesis here

COPDcontrolblank_plots_absolute


# Your existing code to generate plots
combined_plots <- AMRclass.absolute.bars.control + AMRclass.absolute.bars.copd + AMRclass.absolute.bars.blanks

# Rearrange the order of the plots
combined_plots_ordered <- combined_plots[[2]] / combined_plots[[1]] / combined_plots[[3]]

# Save the combined plot
ggsave("Output_files/AMR_abundance_plots/Absolute_abundances/combined_plots.png", combined_plots_ordered, width = 12, height = 8, dpi = 300)
...
ggarrange(AMRclass.absolute.bars.copd, AMRclass.absolute.bars.control,AMRclass.absolute.bars.blanks,
          labels = c("COPD", "Controls", "Blanks"),
          ncol = 3, nrow = 1,
          common.legend = TRUE, legend = "right") 



ggsave("Output_files//AMR_abundance_plots//Absolute_abundances//Combined_adjacent_plot.png", height = 8, width = 4, dpi = 3000)
...
AMRclass.relative.bars_groupedcopdcaco <- bar.graphs.relative(AMRclass.relative, "copdcaco", "ARG_class") +
  ggtitle("Total rarefied, gene length corrected and 16S qPCR corrected resistome relative abundance") +
  labs(fill = "AMR class") +
  theme(axis.text = element_text(size = 15),
        axis.title = element_text(size = 15),
        axis.text.x = element_text(angle = 0, hjust = 0.5),  # Align labels to the center of tick points
        ...
        legend.text = element_text(size = 15),
        legend.position = "right") +
  scale_x_discrete(labels = c("COPD", "Control", "Blank"))  # Set the correct x-axis labels
