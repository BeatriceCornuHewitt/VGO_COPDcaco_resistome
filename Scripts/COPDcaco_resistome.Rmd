...
setwd("/path/to/your/R/project/folder")
```
...
argclusterfile  <- "ResfinderClustersFinal_Resfinder20200127_inclGeneLen+pheno_20201206b.tab"
argmappingfile  <- "02e_TotalResfinderMapped_v3.tab"
# output
psfile          <- "03_VGO-COPD_resistome_v4_phyloseq_object(BCH).RDS"               # filename R Data Serialization (RDS) phyloseq object
psrarfile       <- "03_VGO-COPD_resistome_v4_phyloseq_object_rarefied(BCH).RDS"      # filename RDS phyloseq object rarefied
psfpkfile       <- "03_VGO-COPD_resistome_v4_phyloseq_object_rarefied_FPK(BCH).RDS"  # filename RDS phyloseq object rarefied and gene length corrected
psfpkmfile    <- "03_VGO-COPD_resistome_v4_phyloseq_object_rarefied_FPKM(BCH).RDS" # filename RDS phyloseq object rarefied, gene length corrected and qPCR 16-S corrected 
# folders
...
saveRDS(merged.rar3, file = "merged.rar3.rds")
```
...
```
# Abundance of ARG clusters in COPD, control and blank groups
```{r}
...
write.xlsx(df_copd_ARGclusters, "df_copd_ARGclusters.xlsx", rowNames = FALSE)
write.xlsx(df_control_ARGclusters, "df_control_ARGclusters.xlsx", rowNames = FALSE)
write.xlsx(df_blanc_ARGclusters, "df_blanc_ARGclusters.xlsx", rowNames = FALSE)
```
# Resistome analysis
## Basic numbers
```{r}
## Original data (ARG level)
# Print the number of samples in the dataset
nsamples(Final.ps)
# Number and names of all original ARGs in the dataset
ntaxa(Final.ps)
taxa_names(Final.ps)
# Number and names of all ARGclusters (90% identity) in the dataset
ntaxa(ARGcluster.aggl)
taxa_names(ARGcluster.aggl) # Names are acutally one ARG name chosen for each cluster - trying to solve how to change this if possible
# Number and names of all AMR classes in the dataset
ntaxa(AMRclass.aggl)
taxa_names(AMRclass.aggl) # again...Names are acutally one ARG name chosen for each class - trying to solve how to change this if possible
...
# Examine the column names in the 'sample data' part of the phyloseq object
sample_variables(Final.ps)
# Summary of abundance values of first 6 ARGs in a random sample (sampleID#14587) to get a feel for the data 
head(get_taxa(Final.ps, "14587"))
# Abundance values of a randomly selected ARG (aadD_2_M19465) in the first 6 samples in our dataset
head(get_sample(Final.ps, "aadD_2_M19465"))
# Examine the total read pair counts (volume corrected) for the first 6 samples in the dataset
head(get_variable(Final.ps, "RelativeCorrect_readpair_counts"))
...
# create a table to show the number of genes categorised into each AMR class in this dataset
table('tax_table'(Final.ps)[, "ARG_class"], exclude = NULL)
# This shows us that all ARGs have been assigned to an AMR class (as no NAs present here so no artifacts in the data)
# We have a total of 233 genes which are categorised into 8 AMR classes 
# Only for Fusidicacid was only one gene observed - maybe worth filtering in future? Only 2 genes for Trimethoprim resistance. 

table('tax_table'(ARGcluster.aggl)[, "ARG_class"], exclude = NULL)
```

## Visualise data 
### Bar chart - total AMR class abundance
```{r}
# Create bar plot of sample vs AMR class 
AMRclass.bars <- plot_bar( AMRclass.aggl, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16-S qPCR corrected resistome counts", x="Sample", y="Abundance") +
    theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) ) 
    # scale_y_continuous(limits = c(0,580), expand = c(0, 0))
AMRclass.bars
# # Create an interactive ggplot
# ggplotly(AMRclass.bars, height=600, width=800 
#          +geom_text(size=10))

# COPD cases in a bar plot 
AMRclass.aggl.copd <- AMRclass.aggl
AMRclass.aggl.copd <- prune_samples(sample_data(AMRclass.aggl.copd)$copdcaco == "1", AMRclass.aggl.copd)

AMRclass.bars.copd <- plot_bar( AMRclass.aggl.copd, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16-S qPCR corrected resistome counts, COPD only", x="Sample", y="Abundance") +
    theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) ) 
AMRclass.bars.copd

# Controls in a bar plot
AMRclass.aggl.control <- AMRclass.aggl
AMRclass.aggl.control <- prune_samples(sample_data(AMRclass.aggl.control)$copdcaco == "0", AMRclass.aggl.control)
AMRclass.bars.control <- plot_bar( AMRclass.aggl.control, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16-S qPCR corrected resistome counts, control only", x="Sample", y="Abundance") +
    theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) ) 
AMRclass.bars.control
AMRclass.bars.control

# COPD cases and controls in one plot 
# Firstly remove blanks 
blancs <- c("veldbl16", "veldbl3", "veldbl5")
head(sample.names(AMRclass.relative))
AMRclass.aggl.noblanks <- AMRclass.aggl
AMRclass.aggl.noblanks <-  prune_samples(!sample_names(AMRclass.aggl.noblanks)%in%blancs, AMRclass.aggl.noblanks)
...
AMRclass.bars.case.control <- plot_bar( AMRclass.aggl.noblanks, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16-S qPCR corrected resistome counts", x="Sample", y="Abundance") +
    theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=10 ), title = element_text(size = 20)) 
AMRclass.bars.case.control

AMRclass.bars.case.control <- plot_bar( AMRclass.aggl.noblanks, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16-S qPCR corrected resistome counts", x="Sample", y="Abundance") +
    theme( axis.text = element_text(size=15), axis.title = element_text(size = 15), axis.title.x = element_text(angle = 0), title = element_text(size=20), legend.title = element_text(size=20), legend.text = element_text(size=20))
AMRclass.bars.case.control
```
### Bar chart - relative AMR class abundance 
```{r}
# Create stacked bar chart of AMR classes relative to total ARGs
...
AMRclass.relative.bars <- plot_bar( AMRclass.relative, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16-S qPCR  \ncorrected resistome relative abundance", x="Sample", y="Abundance (%)") 

AMRclass.relative.bars +
    theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) )

#interactive show
ggplotly( AMRclass.relative.bars, height=600, width=800 
          +geom_text(size=15))

# Remove blanks from the stacked bar chart
blancs <- c("veldbl16", "veldbl3", "veldbl5")
head(sample.names(AMRclass.relative))
...
AMRclass.relative.noblanks;AMRclass.relative # the 3 blanks have now been removed from the ps object and this has been named as AMRclass.relative.noblanks

AMRclass.relative.bars.noblanks <- plot_bar(AMRclass.relative.noblanks, fill="ARG_class") +
    labs( title="Total rarefied, gene length corrected and 16-S qPCR  \ncorrected resistome relative abundance", x="Sample", y="Abundance (%)") 

AMRclass.relative.bars.noblanks +
    theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) )
# WvK suggested to create a chart agglomerated on COPD/control status for poster presentation
# code from WvK: 
# toplist <- 10 # variable to set the top x you want. 
# ungrouped_genus <- bar.graphs(ps, "ungrouped", "Genus", toplist)+ ggtitle("Top 10 genus blanco's")

# Creating a chart with COPD/control combined
# AMRclass.relative.bars_groupedcopdcaco <- plot_bar(AMRclass.relative, x= "copdcaco", fill="ARG_class") +
#     labs( title="Total rarefied, gene length corrected and 16-S qPCR  \ncorrected resistome relative abundance", x="COPD or control samples", y="% of total composition")
#   
# AMRclass.relative.bars_groupedcopdcaco + theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) )
#     #scale_y_continuous(limits = c(0,580), expand = c(0, 0))
# 
# # remove OTU lines 
# AMRclass.relative.bars_groupedcopdcaco <- plot_bar(AMRclass.relative, x= "copdcaco", fill="ARG_class") +
#     labs( title="Total rarefied, gene length corrected and 16-S qPCR  \ncorrected resistome relative abundance", x="COPD, control or blank samples", y="% of total composition")
# AMRclass.relative.bars_groupedcopdcaco +
#     theme( axis.text.x = element_text( angle=270, vjust=0.4, hjust=0, size=8 ) )+
#    geom_bar( stat="identity",position="stack")
# 
# Remove blanks from the ps object "AMRclass.relative"
blancs <- c("veldbl16", "veldbl3", "veldbl5")
head(sample.names(AMRclass.relative))
AMRclass.relative.noblanks <- AMRclass.relative
AMRclass.relative.noblanks <-  prune_samples(!sample_names(AMRclass.relative)%in%blancs, AMRclass.relative)
...
# rename the 0s and 1s to control and case 
AMRclass.relative.noblanks@sam_data$copdcaco  <- as.factor(AMRclass.relative.noblanks@sam_data$copdcaco )
str(AMRclass.relative.noblanks@sam_data$copdcaco)
AMRclass.relative.noblanks@sam_data$copdcaco <- 
recode_factor(AMRclass.relative.noblanks@sam_data$copdcaco,"1"="COPD", "0"="control")
# recode_factor() function changes the order of the levels to match the order of the replacements - I manually checked to ensure that COPD and controls have been labelled correctly.

# using AB's "bar.graph" function - BCH slightly edited to change axes titles
# Updated bar.graphs function with modified legend title
bar.graphs <- function(physeq, var1 = "ungrouped", level, displayx = 10, filetype = ".jpg") {
  if (level == "Species") {
...
# Updated code with modified legend title
AMRclass.relative.bars_groupedcopdcaco.noblanks <- bar.graphs(AMRclass.relative.noblanks, "copdcaco", "ARG_class") +
  ggtitle("Total rarefied, gene length corrected and 16-S \nqPCR corrected resistome relative abundance") +
  labs(fill = "AMR class") +
  ...
  scale_x_discrete(labels = c("Control", "COPD"))

AMRclass.relative.bars_groupedcopdcaco.noblanks


#interactive plot
ggplotly( AMRclass.relative.bars_groupedcopdcaco.noblanks, height=600, width=800 
          +geom_text(size=15))

ggsave("AMRclass.relative.bars_groupedcopdcaco.noblanks.png",width = 7, height = 5,dpi = 3000)
...
#### ARG level
```{r}
# Heatmap as a graphical representation of the relative abundance tables
# Default heatmap settings:
plot_heatmap(Final.ps, xlab = "ARGs", ylab = "SampleIDs", main = "Heatmap all samples")
```

```{r}
# Elaborating on the heatmap
...
ARG.dataframe.prop
ARG.dataframe.prop[1:3, 1:3]
heatmap(as.matrix(ARG.dataframe.prop), Rowv = NA, Colv = NA)
...
heatmap1 <- heatmap(as.matrix(ARG.dataframe.prop), Rowv = as.dendrogram(sample.clust), Colv = NA, margins = c(10, 3))
```

```{r}
# add column dendogram to heatmap 
# calculate the Bray-Curtis dissimilarity matrix on ARGs in the dataset
ARG.dataframe.BCdist.g <- vegdist(t(ARG.dataframe.prop), method = "bray")

# perform hierarchical cluster analysis using the BC dissimilarities between the ARGs (columns in dataset)
col.clust <- hclust(ARG.dataframe.BCdist.g, "aver")

# create the heatmap with row and column dendograms
heatmap2 <- heatmap(as.matrix(ARG.dataframe.prop), Rowv = as.dendrogram(sample.clust), Colv = as.dendrogram(col.clust), margins = c(11, 5), xlab = "ARGs", ylab = "SampleIDs", main = "Hierarchical clustering heatmap")
```
...
heatmap.colouredsamples <- heatmap(as.matrix(ARG.dataframe.prop), Rowv = as.dendrogram(sample.clust), Colv = as.dendrogram(col.clust), RowSideColors = COPDcontrolblank, margins = c(11, 5), xlab = "ARGs", ylab = "SampleIDs", main = "Hierarchical clustering heatmap")

```

```{r}
# Change the overall heatmap colours - nice website to explain (https://www.r-graph-gallery.com/215-the-heatmap-function.html)

# Using native colour palette from R - e.g. terrain palette:
heatmap.diffcolour1 <- heatmap(as.matrix(ARG.dataframe.prop), Rowv = as.dendrogram(sample.clust), Colv = as.dendrogram(col.clust), col = terrain.colors(256), RowSideColors = COPDcontrolblank, margins = c(11, 5), xlab = "ARGs", ylab = "SampleIDs", main = "Hierarchical clustering heatmap")
# Using native colour palette from R - e.g. cm palette:
heatmap.diffcolour1 <- heatmap(as.matrix(ARG.dataframe.prop), Rowv = as.dendrogram(sample.clust), Colv = as.dendrogram(col.clust), col = cm.colors(256), RowSideColors = COPDcontrolblank, margins = c(11, 5), xlab = "ARGs", ylab = "SampleIDs", main = "Hierarchical clustering heatmap")

# Using the Rcolorbrewer palette (colorRampPalette is in the RColorBrewer package)
# view the palette options: 
par(mar=c(3,4,2,2))
display.brewer.all()
# Try out a colour palette from here - e.g. 'blues' 
heatmap.diffcolour1 <- heatmap(as.matrix(ARG.dataframe.prop), Rowv = as.dendrogram(sample.clust), Colv = as.dendrogram(col.clust), col = colorRampPalette(brewer.pal(8, "Blues"))(25), RowSideColors = COPDcontrolblank, margins = c(11, 5), xlab = "ARGs", ylab = "SampleIDs", main = "Hierarchical clustering heatmap")

# Creat a heatmap with text (i.e. values on top of the heatmap)? Maybe unnecessary?  https://microbiome.github.io/tutorials/Heatmap.html 
```
#### ARG cluster 90% level 
##### Standard R heatmap
```{r}
# Create a heatmap now on the cluster90 level as this is a better representation of the data 
...
ARGcluster.dataframe
colnames(ARGcluster.dataframe)
list(colnames(ARGcluster.dataframe))
...
ARGcluster.dataframe.prop[1:3, 1:3]
heatmap(as.matrix(ARGcluster.dataframe.prop), Rowv = NA, Colv = NA) # Simple heatmap on cluster level
...
# perform hierarchical cluster analysis using the BC dissimilarities 
# each sample is assigned to its own cluster and then the algorithm proceeds iteratively, at each stage joining the two most similar clusters, continuing until there is just a single cluster.
sample.BCclustering <- hclust(ARGCluster.BCdist, "aver")

# perform hierarchical cluster analysis using the BC dissimilarities between the AMR classes (columns in dataset)
ARGCluster.BCclustering <- hclust(ARGCluster.BCdist, "aver")

# Create a heatmap at the ARG cluster level
heatmap.ARGClusterlevel <- heatmap(as.matrix(ARGcluster.dataframe.prop), Rowv = as.dendrogram(sample.BCclustering), RowSideColors = COPDcontrolblank, margins = c(11, 5), xlab = "ARGCluster", ylab = "SampleIDs", main = "Hierarchical clustering heatmap at 90% ARG cluster level")

# Note that the samples are not in the same order as that in the matrix - heatmap() reorders both variables and observations using the BC clustering algorithm: it computes the distance between each pair of rows and columns and try to order them by similarity.
```
##### Pheatmap at ARG cluster level
```{r}
#Heatmap at the 90% cluster level 
# Using tutorial: https://slowkow.com/notes/pheatmap-tutorial/
# Use the data frame which I created previously for ARG cluster level - inspect this dataframe again to re-acquaint with data
ARGcluster.dataframe.prop
ARGcluster.dataframe.prop[1:3, 1:3]
...
## Check for contaminants in the dataset - using decontam package
```{r}
...
**
### Alpha diversity indices - ROUNDING METHOD (incorrect as we lose information)
```{r}
# ROUNDING METHOD
# Firstly we need to convert fractional ARG counts to whole numbers by ROUNDING 
head(otu_table(Final.ps))
head(sample_data(Final.ps))
head(tax_table(Final.ps))

# Round ARG counts to nearest whole number then update the ps object with this new otu_table 
rounded.ARGcounts <- ceiling(as(otu_table(Final.ps), "matrix")) 
# check that rounding has been performed correctly by comparing with original ps ARG corrected counts
head(rounded.ARGcounts)
head(otu_table(Final.ps))
# rounding has been performed correctly

# Create a new ps object with these rounded ARG counts
Final.ps.round <- phyloseq( otu_table( rounded.ARGcounts, taxa_are_rows=FALSE ), tax_table( as.matrix(ARGclusters)))
# somehow the apostrophes get converted from df to matrix. Therefore we import again directly the matrix into the ps object to preserve names!
Final.ps.round@otu_table@.Data <- as.matrix(rounded.ARGcounts)
# align meta data and merge
metadata <- metadata[ match( sample_names(Final.ps.round), rownames(metadata) ), ]
# final PS object
Final.ps.round@sam_data <- sample_data(metadata)
# Remove duplicate sample (13674) from the new rounded dataset
Final.ps.round <- prune_samples(sample_names(Final.ps.round) != "13674", Final.ps.round) 
Final.ps.round# Plot all alpha diversity measures
...
# Here is the default graphic produced by the plot_richness function on the rounded dataset. 
plot_richness(Final.ps.round)
# Unsure what the bars then mean here  - how can the alpha diversity vary per sample - these bars are present for Chao1 and ACE but not other alpha diversity measures
# There are many alpha diversity indices which each reflect different aspects of community heterogeneity.

# Plot 4 chosen diversity indices (observed,Chao1, shannon and simpson) 
plot_richness(Final.ps.round, measures=c("Observed", "Chao1", "Shannon", "Simpson"))

# Plot alpha diversity scores for COPD, controls (and blanks) separately
plot_richness(Final.ps.round, x="copdcaco", measures=c("Observed", "Chao1", "Shannon", "Simpson"))

# Create table with alpha diversity measures 
library(microbiome)
table.alphadivers <- microbiome::alpha(Final.ps.round, index = "all", zeroes = TRUE)
table.alphadivers

Observed.diversity <- table.alphadivers$observed
Chao1.diversity <- table.alphadivers$chao1
Shannon.diversity <- table.alphadivers$diversity_shannon
Simpson.diversity <- table.alphadivers$diversity_inverse_simpson

#Now, get the metadata (sample_data) from the phyloseq object
metadata.final.ps.round <- sample_data(Final.ps.round)
head(metadata.final.ps.round)
#Add the diversity tables to the metadata
metadata.final.ps.round$Observed <- Observed.diversity
metadata.final.ps.round$Chao1 <- Chao1.diversity
metadata.final.ps.round$Shannon <- Shannon.diversity
metadata.final.ps.round$Inverse.Simpson <- Simpson.diversity
#check whether this has been added to the metadata table
head(metadata.final.ps.round)
head(sample_data(Final.ps.round))
# yes these 3 columns have been added to the metadata of the rounded phyloseq object, but not when we do sample_data(final.ps.round) - not sure why this is... 

# Create new ps object including alpha diversity measures
Final.ps.round.alpha <- phyloseq( otu_table( rounded.ARGcounts, taxa_are_rows=FALSE ), tax_table( as.matrix(ARGclusters)))
# somehow the apostrophes get converted from df to matrix. Therefore we import again directly the matrix into the ps object to preserve names!
Final.ps.round.alpha@otu_table@.Data <- as.matrix(rounded.ARGcounts)
# align meta data and merge
metadata.alpha <- metadata.final.ps.round[ match( sample_names(Final.ps.round.alpha), rownames(metadata.final.ps.round) ), ]
# final PS object
Final.ps.round.alpha@sam_data <- sample_data(metadata.final.ps.round)
# Remove duplicate sample (13674) from the new rounded dataset
Final.ps.round.alpha <- prune_samples(sample_names(Final.ps.round) != "13674", Final.ps.round.alpha)
Final.ps.round.alpha
# Check new metadata has been added to the ps object
head(metadata.final.ps.round)
head(sample_data(Final.ps.round.alpha))
# Yes (24 variables now - alpha diversity measures have been added)

# Make a histograms of the different diversity scores throughout the dataset
hist(Observed.diversity, main="Observed diversity", xlab="", breaks=10)
hist(Chao1.diversity, main="Chao1 diversity", xlab="", breaks=10)
hist(Shannon.diversity, main="Shannon diversity", xlab="", breaks=10)
hist(Simpson.diversity, main="Simpson diversity", xlab="", breaks=10)

# Create 2x2 plot environment so that we can see all 4 metrics at once. 
par(mfrow = c(2, 2))
#Then plot each histogram to visualise all 4 together:
hist(Observed.diversity, main="Observed diversity", xlab="", breaks=15)
hist(Chao1.diversity, main="Chao1 diversity", xlab="", breaks=15)
hist(Shannon.diversity, main="Shannon diversity", xlab="", breaks=15)
hist(Simpson.diversity, main="Simpson diversity", xlab="", breaks=15)


# Create boxplots for COPD/control for the different alpha diversity measures
# Create 2x2 plot environment so that we can see all 4 metrics at once. 
par(mfrow = c(2, 2))
# Make the 4 boxplots:
ggplot(metadata.final.ps.round, aes(x = copdcaco, y = Observed.diversity)) + geom_boxplot()
ggplot(metadata.final.ps.round, aes(x = copdcaco, y = Chao1.diversity)) + geom_boxplot()
ggplot(metadata.final.ps.round, aes(x = copdcaco, y = Shannon.diversity)) + geom_boxplot()
ggplot(metadata.final.ps.round, aes(x = copdcaco, y = Simpson.diversity)) + geom_boxplot()

# check normality of these distributions across all the samples
shapiro.test(Observed.diversity)
shapiro.test(Chao1.diversity)
shapiro.test(Shannon.diversity)
shapiro.test(Simpson.diversity)
# Observed and Chao1 appear normally distributed, Shannon and Simpson not normally distributed...
```

### Alpha diversity indices - MULTIPLY BY BILLION METHOD
**
- The rounding method used above causes a loss of data
...
head(Final.ps.multiplybillion@otu_table@.Data)

# Firstly I will rename the codes "1", "0" and "blanc" as COPD, control and blank respectively so that the graphs are easier to interpret - directly manipulate the new PS matrix
...
# View the new variable names
names(Final.ps.multiplybillion@sam_data$copdcaco)
...
plot_richness(Final.ps.multiplybillion, measures=c("Richness", "Shannon", "Chao1", "Observed", "Simpson"))

# Plot alpha diversity scores for COPD, controls (and blanks) separately
Alphadiversity_scatter<- plot_richness(Final.ps.multiplybillion, x="copdcaco", measures=c("Shannon", "Chao1", "Observed", "Simpson", "Evenness"))
Alphadiversity_scatter

# Add colour to the scatterplot
mycols3 <- c("COPD"="red", "control"="blue", "blank"="green")
Alphadiversity_scatter.colour <- plot_richness(Final.ps.multiplybillion, x="copdcaco", measures=c("Observed", "Chao1", "Shannon", "Simpson"), color="copdcaco")
Alphadiversity_scatter.colour + scale_color_manual(values = mycols3, aesthetics = c("color", "fill"))
...
#check whether this has been added to the metadata table
head(sample_data(Final.ps.multiplybillion))
# Yes these columns have been added to the metadata of the rounded phyloseq object now

# Make a histograms of the different diversity scores throughout the dataset
# Create 2x2 plot environment so that we can see all 4 metrics at once. 
par(mfrow = c(2, 2))
hist(sample_data(Final.ps.multiplybillion)$shannon, main="Shannon diversity", xlab="", breaks=15)
hist(sample_data(Final.ps.multiplybillion)$Chao1, main="Chao1 diversity", xlab="", breaks=15)
...
shapiro.test(sample_data(Final.ps.multiplybillion)$Chao1)
...
# Observed and Chao1 appear normally distributed, Shannon and Inv Simpson not normally distributed...

# Create boxplots for COPD/control/blanks for the different alpha diversity measures
# Create 2x2 plot environment so that we can see all 4 metrics at once. 
# Firstly get the metadata (sample_data) from the phyloseq object
metadata.final.ps.billion <- sample_data(Final.ps.multiplybillion)
head(metadata.final.ps.billion)
par(mfrow = c(2, 2))
# Make the 4 boxplots:
ggplot(metadata.final.ps.billion, aes(x = copdcaco, y = shannon )) + geom_boxplot(fill= c("blue","red", "green"))
ggplot(metadata.final.ps.billion, aes(x = copdcaco, y = Chao1)) + geom_boxplot(fill= c("blue","red", "green"))
...
Final.ps.multiplybillion.noblanks;Final.ps.multiplybillion

# Plot alpha diversity scores for COPD, controls (and blanks) separately
Alphadiversity_scatter.noblanks<- plot_richness(Final.ps.multiplybillion.noblanks, x="copdcaco", measures=c("Shannon", "Chao1", "Observed", "Simpson", "Evenness"))
Alphadiversity_scatter.noblanks

# Add colour to the scatterplot
mycols4 <- c("COPD"="red", "control"="blue")
Alphadiversity_scatter.noblanks.colour <- plot_richness(Final.ps.multiplybillion.noblanks, x="copdcaco", measures=c("Observed", "Chao1", "Shannon", "Simpson"), color="copdcaco")
Alphadiversity_scatter.noblanks.colour + scale_color_manual(values = mycols4, aesthetics = c("color", "fill"))

# Make a histograms of the different diversity scores throughout the dataset
# Create 2x2 plot environment so that we can see all 4 metrics at once. 
par(mfrow = c(2, 2))
hist(sample_data(Final.ps.multiplybillion.noblanks)$shannon, main="Shannon diversity", xlab="", breaks=15)
hist(sample_data(Final.ps.multiplybillion.noblanks)$Chao1, main="Chao1 diversity", xlab="", breaks=15)
...
shapiro.test(Final.ps.multiplybillion.noblanks@sam_data$shannon)
shapiro.test(sample_data(Final.ps.multiplybillion.noblanks)$Chao1)
...
# Shannon appears (just) normally distributed but the others are not
...
Shannon.diversity.plot <- ggplot(metadata.final.ps.billion.noblanks, aes(x = copdcaco, y = shannon)) + 
  geom_boxplot(fill= c("blue","red"),  alpha=0.5, outlier.shape = NA, fatten= 4)+ # alpha argument makes fill more transparent, fatten argument makes median line thicker,
...
# Create boxplots for COPD and control for the different alpha diversity measures
Shannon.diversity.plot.new <- ggplot(metadata.final.ps.billion.noblanks, aes(x = copdcaco, y = shannon)) + 
...
Shannon.diversity.plot.new <- ggplot(metadata.final.ps.billion.noblanks, aes(x = copdcaco, y = shannon)) +
  geom_boxplot(aes(fill = copdcaco, color = copdcaco), alpha = 0.5, outlier.shape = NA, size=2) + 
  scale_fill_manual(values = c("coral2","cornflowerblue")) + # set the fill colors
  scale_color_manual(values = c("blue", "red")) + # set the border colors
  ggtitle("Shannon diversity") +
  theme(plot.title = element_text(size = 50, family = "Arial"), axis.title = element_text(size = 35, family = "Arial"), axis.text = element_text(size = 35, family = "Arial")) +
  theme(plot.title = element_text(hjust = 0.5)) + #puts title in centre 
  xlab("Study population") +
  ylab("Shannon diversity index") +
  geom_point(position = position_jitter(width = 0.2, height = 0), size = 6, alpha = 0.8, aes(color = copdcaco)) +
  scale_color_manual(values = c("coral2","cornflowerblue"))# adds jittered points to plot, not in matched colours yet though

# Show the plot
Shannon.diversity.plot.new
   

# Chao1.diversity.plot <- ggplot(metadata.final.ps.billion.noblanks, aes(x = copdcaco, y = Chao1)) + geom_boxplot(fill= c("blue","red"), outlier.size = 6)+ ggtitle("Chao1 diversity") + theme(plot.title= element_text(size=50), axis.title= element_text(size =40), axis.text = element_text(size = 35))

Chao1.diversity.plot <- ggplot(metadata.final.ps.billion.noblanks, aes(x = copdcaco, y = Chao1)) + 
  geom_boxplot(fill= c("blue","red"),  alpha=0.5, outlier.shape = NA, fatten= 4)+ # alpha argument makes fill more transparent, fatten argument makes median line thicker,
...
Shannon.diversity.plot +  stat_compare_means(method = "t.test", size=11, label.x = 0.6, label.y = 3.5)
Chao1.diversity.plot +  stat_compare_means(method = "wilcox.test", size=11,label.x = 0.6, label.y = 3.5)
Observed.diversity.plot+  stat_compare_means(method = "wilcox.test", size=11,label.x = 0.6, label.y = 100)
Simpson.diversity.plot+  stat_compare_means(method = "wilcox.test", size=11, label.x = 0.6, label.y = 0.5)


Shannon.diversity.plot.new +  stat_compare_means(method = "t.test", size=11, label.x = 0.6, label.y = 3.5)
...
## Beta diversity - redone at 90% cluster level
...
### Ordinations: PCoA 
```{r}
# I will construct principal coordinates analysis based on Bray-Curtis dissimilarities. These measures are based on abundance or read count data. It generates differences in microbial abundances between two samples (e.g., at species level) which are from 0 to 1 (where 0= both samples share the same species at exactly the same abundances and 1= both samples have complete different species abundances).
ARGcluster.aggl.PCoA <- ordinate(ARGcluster.aggl, method="PCoA", distance="bray")
ARGcluster.aggl.PCoA.plot <- plot_ordination( ARGcluster.aggl, ARGcluster.aggl.PCoA, color="copdcaco", shape="copdcaco" ) +
    ggtitle("Bray-Curtis PCoA (fully corrected and rarefied ARG counts, 90% identity clustered ) - per sample") +
    geom_point(size = 3) +
    geom_text( aes( label=row.names(ARGcluster.aggl@sam_data),hjust=-0.3, vjust=-0.5 ) )
ARGcluster.aggl.PCoA.plot

ARGcluster.aggl.PCoA.plot + stat_ellipse() # add ellipses to the PCoA

# Show dominant eigen planes
plot_scree( ARGcluster.aggl.PCoA )

# Remove blanks and re-do the plots 
blancs <- c("veldbl16", "veldbl3", "veldbl5")
head(sample.names(ARGcluster.aggl))
ARGcluster.aggl.noblanks <- ARGcluster.aggl
ARGcluster.aggl.noblanks <-  prune_samples(!sample_names(ARGcluster.aggl)%in%blancs, ARGcluster.aggl)
ARGcluster.aggl.noblanks;ARGcluster.aggl

# Re-do plots without blanks
ARGcluster.aggl.PCoA.noblanks <- ordinate(ARGcluster.aggl.noblanks, method="PCoA", distance="bray")

ARGcluster.aggl.PCoA.plot.noblanks <- plot_ordination( ARGcluster.aggl.noblanks, ARGcluster.aggl.PCoA.noblanks, color = "copdcaco", shape="copdcaco") +
    ggtitle("Bray-Curtis PCoA - COPD vs control") +
  labs(legend="COPD/control status")+
    geom_point(size =8) +
    geom_text( aes( label=row.names(ARGcluster.aggl.noblanks@sam_data),hjust=-0.3, vjust=-0.5 ) ) +
  theme(plot.title = element_text(size = 30))+ # adjusting size of title text
  theme(axis.text=element_text(size=20),
        axis.title=element_text(size=20)) + # adjusting size of axis text
  theme(legend.text=element_text(size=20),
        legend.title = element_text(size=20))+ # adjusting size of legend text
  stat_ellipse(geom = "polygon", alpha = 1/6, aes(fill = copdcaco))# adds ellipses to the PCoA, fills ellipses with colour according to copdcaco status, colour transparency=1/4 of strength 
ARGcluster.aggl.PCoA.plot.noblanks

# Show dominant eigen planes
plot_scree(ARGcluster.aggl.PCoA.noblanks)

# Create a PCoA per enrichment pool
ARGcluster.aggl.PCoA.plot2.noblanks <- plot_ordination( ARGcluster.aggl.noblanks, ARGcluster.aggl.PCoA.noblanks, color="Enrich_pool_num", shape="copdcaco" ) +
    ggtitle("Bray-Curtis PCoA - per enrichment pool")  + 
    geom_point(size = 3) +
    geom_text( aes( label=ARGcluster.aggl.noblanks@sam_data$Enrich_pool_num, size=10, hjust=-0.3, vjust=-0.5) )+
  ...
  theme(legend.text=element_text(size=20),
    ...
    stat_ellipse()
ARGcluster.aggl.PCoA.plot2.noblanks

# Redone for poster for VSD 
# Firstly I will rename the codes "1", "0" and "blanc" as COPD, control and blank respectively so that the graphs are easier to interpret - directly manipulate the new PS matrix
ARGcluster.aggl.noblanks@sam_data$copdcaco <- revalue(ARGcluster.aggl.noblanks@sam_data$copdcaco, c('1' ='COPD', '0' = 'control'))

ARGcluster.aggl.PCoA.noblanks.new <- ordinate(ARGcluster.aggl.noblanks, method="PCoA", distance="bray")

ARGcluster.aggl.PCoA.plot.noblanks.new <- plot_ordination(ARGcluster.aggl.noblanks, ARGcluster.aggl.PCoA.noblanks, color = "copdcaco", shape = "copdcaco") +
  ggtitle("Bray-Curtis PCoA - COPD vs control") +
  labs(legend = "COPD/control status") +
  geom_point(aes(color = copdcaco), size = 3) + # coloring points based on copdcaco variable
  scale_color_manual(values = c("coral2", "cornflowerblue")) + # assigning blue to COPD and red to control
  geom_text(aes(label = row.names(ARGcluster.aggl.noblanks@sam_data), hjust = -0.3, vjust = -0.5)) +
  ...
  scale_fill_manual(values = c("coral2", "cornflowerblue")) + # assigning blue to COPD and red to control
  scale_color_manual(values = c("coral2", "cornflowerblue")) # assigning blue to COPD and red to control

ggsave("ARGcluster.aggl.PCoA.plot.noblanks.new.png", plot = ARGcluster.aggl.PCoA.plot.noblanks.new, width = 7, height = 6, dpi = 300)
```
### Ordinations: NMDS
```{r}
#NMDS with blanks included
ARGcluster.aggl.NMDS <- ordinate( ARGcluster.aggl, method="NMDS", distance="bray", autotransform=FALSE, trymax=100)
ARGcluster.aggl.NMDS.plot <- plot_ordination( ARGcluster.aggl, ARGcluster.aggl.NMDS, color="bedr_1000_bvb12", shape="copdcaco") +
    ggtitle("Ordination corrected ARG counts") +
    geom_point(size = 3) +
    geom_text( aes( label=row.names(ARGcluster.aggl@sam_data),hjust=-0.3, vjust=-0.5 ) )

ARGcluster.aggl.NMDS.plot
stressplot(ARGcluster.aggl.NMDS)
```

```{r}
# NMDS with blanks removed
ARGcluster.aggl.noblanks.NMDS <- ordinate( ARGcluster.aggl.noblanks, method="NMDS", distance="bray", autotransform=FALSE, trymax=100)

ARGcluster.aggl.noblanks.NMDS.plot <- plot_ordination( ARGcluster.aggl.noblanks, ARGcluster.aggl.noblanks.NMDS, color="copdcaco", shape="copdcaco") +
    ggtitle("NMDS ordination - bray-curtis (90% identity clustered)") +
    geom_point(size = 3) +
    geom_text( aes( label=row.names(Final.ps.noblanks@sam_data),hjust=-0.3, vjust=-0.5 ) )

ARGcluster.aggl.noblanks.NMDS.plot

# Add ellipses to NMDS
ARGcluster.aggl.noblanks.NMDS.plot + stat_ellipse()
stressplot(ARGcluster.aggl.noblanks.NMDS)
goodness(ARGcluster.aggl.noblanks.NMDS)

```

### NMDS or PCoA - Shepard plots 
```{r}
# Is NMDS or PCoA better for this data? 
# I will use a Shepard plot to determine which is a better fit to the data (see p.156 of "Multivariate analyses in microbial ecology" )
# A Shepard diagram is one way of assessing if an ordination produced an accurate visualization. It is a scatterplot of distances between data points. On the x-axis, we plot the original distances. On the y-axis, we plot the distances output by a dimension reduction algorithm.
#I think that NMDS is probably a better option for my data, it is stated in "Multivariate analyses in microbial ecology" that "Although NMDS is more computer intensive than PCoA, it is generally better at compressing the distance relationships among objects into a few dimensions. This is because NMDS can always lead to a Euclidean representation even for non-Euclidean embeddable distances (Legendre & Le- gendre, 1998). NMDS and PCoA can be compared using Shepard diagrams to decide which technique better represents the original distances" 
# The shepard plot for the NMDS has already been done (same as stressplot)
stressplot(ARGcluster.aggl.noblanks.NMDS)
goodness(ARGcluster.aggl.noblanks.NMDS)



# I need to create a Shepard plot for the PCoA 
require(devtools)
install_github("Displayr/flipDimensionReduction")
usethis::browse_github_pat()

# Shepard plot for PCoA 
# load required packages for shepard plot of PCoA 
# I need an older version of the package "asbio" which I will load manually 
require(devtools)
packageurl <- "https://cran.r-project.org/src/contrib/Archive/asbio/asbio_0.3-42.tar.gz"
install.packages(packageurl, repos=NULL, type="source")
library(asbio)
# Extract abundance matrix from phyloseq object
ARGclusters.for.beta.div.shep.plot <- as(otu_table(ARGcluster.aggl.noblanks), "matrix")
# transform into a data frame
ARG.cluster.dataframe.shep.plot <-as.data.frame(ARGclusters.for.beta.div.shep.plot)
ARG.cluster.dataframe.shep.plot
BC.distance.no.blanks <- vegdist(ARG.cluster.dataframe.shep.plot, method =  "bray")
Shep.comp(BC.distance.no.blanks, max.dim = 35)
# This matrix has three columns containing R^2 values for for raw, linear, and non-metric. 
# The function Shep.comp in asbio creates calculates three sorts correlations coefficients across all specified PCoA dimensionalities. 1) Pearson's correlations of observed distances (in the distance matrix) and fitted distances (in a PCoA ordination); i.e. "raw fits." 2) Pearson's correlations between the monotonic fitted line and the observed distances "linear fits". 3) A correlation based on stress. The final type of correlation has been called "non-metric" fit 
PCoA.shep.plot.df <- as.data.frame(PCoA.shep.plot)
plot(PCoA.shep.plot.df$R.stress)
PCoA.shep.plot.df$R.stress
...
BC.distance <- vegdist(ARG.cluster.dataframe, method =  "bray")

# Remove blanks from the df
ARG.cluster.dataframe.noblanks <- ARG.cluster.dataframe
ARG.cluster.dataframe.noblanks <- ARG.cluster.dataframe[-c(70, 71, 72),] 
  
# Calculate the BC distances 
ARGClust.BC.distances <- vegdist(ARG.cluster.dataframe.noblanks, method =  "bray")
...
Betadisper.ARGcluster <- betadisper(ARGClust.BC.distances, ARG.cluster.dataframe.noblanks$copdcaco)
Betadisper.ARGcluster

# Perform an anova using the group dispersions.
Anova_betadisper <- anova(Betadisper.ARGcluster)
...
Permanova_results<- adonis2(ARGClust.BC.distances~ ARG.cluster.dataframe.noblanks$copdcaco, permutations = 999)
...
## Association between resistome and bacteriome - procrustes
```{r}
...
getwd()
microbiome.ps <- readRDS("ps_COPD_d2_f_01%in2samples_08APR2021")
...
get_taxa(ARGcluster.aggl, "19586") # There are some ARGs detected - how is this possible - asking AB about this...
...
# HELLINGER TRANSFORMATION ( of microbiome data - not sure if suitable for resistome as already normalised this and not compositional data )
...
# # Resistome data: 
# Resistome.ps.noblanks.hellinger<- Resistome.ps.noblanks
# otu_table(Resistome.ps.noblanks.hellinger) <-otu_table(decostand(otu_table(Resistome.ps.noblanks.hellinger), method = "hellinger"), taxa_are_rows=FALSE)

# NMDS procrustes
# NMDS on microbiome data 
microbiome.NMDS <- ordinate( microbiome.ps.genus.hell, method="NMDS", distance="bray", autotransform=FALSE, trymax=100)
microbiome.NMDS.plot <- plot_ordination( microbiome.ps.genus.hell, microbiome.NMDS, color="copdcaco", shape="copdcaco") +
    ggtitle("Microbiome NMDS") +
    geom_point(size = 3) +
    geom_text( aes( label=row.names(microbiome.ps.final.noblanks.hellinger@sam_data),hjust=-0.3, vjust=-0.5 ) )+
  stat_ellipse()
microbiome.NMDS.plot
# Extract the data from the NMDS plot for use in the procrustes analysis
microbiome.procrustes.data <- microbiome.NMDS.plot$data[, 1:2]
# Get rid of T1 from ID name so that it matches the resistome data
rownames(microbiome.procrustes.data)
rownames(microbiome.procrustes.data) <- stringr::str_replace(rownames(microbiome.procrustes.data), "T\\d", replacement = "")
rownames(microbiome.procrustes.data) #'T1' has now been removed from the 
# NMDS on resistome data
resistome.NMDS <- ordinate(Resistome.ps.noblanks, method="NMDS", distance="bray", autotransform=FALSE, trymax=100)
resistome.NMDS.plot <- plot_ordination( Resistome.ps.noblanks, resistome.NMDS, color="copdcaco", shape="copdcaco") +
    ggtitle("Resistome NMDS Ordination corrected ARG counts") +
    geom_point(size = 3) +
    geom_text( aes( label=row.names(Resistome.ps.noblanks@sam_data),hjust=-0.3, vjust=-0.5 ) )+
  stat_ellipse()

resistome.procrustes.data <- resistome.NMDS.plot$data[, 1:2]

# Run the procrustes on the NMDS ordinations - see tutorial: https://john-quensen.com/tutorials/procrustes-analysis/ 
procrustes.micro.resis.NMDS.symm <- procrustes(microbiome.procrustes.data, resistome.procrustes.data, 
                                          symmetric = TRUE, 
                                          permutations = 9999)

# Results of procrustes
summary(procrustes.micro.resis.NMDS.symm)
print(procrustes.micro.resis.NMDS.symm)
...
plot(procrustes.micro.resis.NMDS.symm,
     kind = 1,
     main = "Procrustes plot - NMDS microbiome/resistome",
     xlab = "NMDS 1",
     ylab = "NMDS 2")
...
plot(procrustes.micro.resis.NMDS.symm, kind =2, main = "Procrustes error plot - NMDS microbiome/resistome")
...
protest(microbiome.procrustes.data, resistome.procrustes.data, scores = "sites", permutations = 9999) # correlation coefficient = 0.4154, p= 0.001
...
# Only need to 'randomise' one of the procrustesa input datasets (here I do microbiome)
microbiome.NMDS.random <- microbiome.procrustes.data
row.names(microbiome.procrustes.data)
row.names(microbiome.NMDS.random)

row.names(microbiome.NMDS.random) <- sample(row.names(microbiome.NMDS.random))
microbiome.NMDS.random <- microbiome.NMDS.random[ match( row.names(resistome.procrustes.data), row.names(microbiome.NMDS.random) ), ]
microbiome.NMDS.random
table(row.names(resistome.procrustes.data),row.names(microbiome.procrustes.data)) # check row names are the same - yes they are

pro.NMDS.r <- procrustes(microbiome.NMDS.random, resistome.procrustes.data, symmetric = TRUE, permutations= 9999)
...
plot(pro.NMDS.r,
     kind = 1,
     main = "Procrustes plot randomised - NMDS microbiome/resistome",
     xlab = "NMDS 1",
     ylab = "NMDS 2")

# Kind 2 plots show the residuals for each sample. This allows identification of samples with the worst fit. The horizontal lines, from bottom to top, are the 25% (dashed), 50% (solid), and 75% (dashed) quantiles of the residuals.
plot(pro.NMDS.r, kind =2)
# Significance testing using 'protest' - this is a permutational test of the significance of the procrustes result based on the correlation from a symmetric procrustes analysis 
protest(microbiome.NMDS.random, resistome.procrustes.data, scores = "sites", permutations = 9999) # correlation coefficient = 0.203, p= 0.112 (non-significant)- as expected, when we compare the resistome and microbiome of random samples within this data, we do not see a correlation between resistome and microbiome (p<0.05 )


# PCoA  
# Firstly ordinate the data 
microbiome.PCoA <- ordinate( microbiome.ps.genus.hell, method="PCoA", distance="bray")
microbiome.PCoA.plot <- plot_ordination( microbiome.ps.genus.hell, microbiome.hell.PCoA, color="copdcaco", shape="copdcaco" ) +
...
rownames(microbiome.procrustes.PCoA.data) #'T1' has now been removed from the 
...
microbiome.PCoA.random
table(row.names(resistome.procrustes.data),row.names(microbiome.procrustes.data)) # check row names are the same - yes they are

pro.PCoA.r <- procrustes(microbiome.PCoA.random, resistome.procrustes.PCoA.data, symmetric = TRUE, permutations= 9999)
...
copdcaco.df.noblanks <- subset(copdcaco_df, copdcaco_df$COPD.control.blank!= "blanc") 
copdcaco.df.noblanks.no19586 <- copdcaco.df.noblanks[row.names(copdcaco.df.noblanks) != "19586", , drop = FALSE]

# NMDS 
# Normal NMDS procrustes 
NMDS.pro.df <- data.frame(rda1=procrustes.micro.resis.NMDS.symm$Yrot[,1], rda2=procrustes.micro.resis.NMDS.symm$Yrot[,2], xrda1 = procrustes.micro.resis.NMDS.symm$X[,1], xrda2 = procrustes.micro.resis.NMDS.symm$X[,2])
NMDS.pro.df$copdcontrol <- copdcaco.df.noblanks.no19586$COPD.control.blank
# Create the ggplot 
NMDS.pro.plot <- ggplot(NMDS.pro.df) +
...
NMDS.pro.plot + labs(title = "NMDS procrustes", y="NMDS second axis", x = "NMDS first axis", subtitle = "Correlation: 0.44, Significance: 1e-04")

# Random NMDS procrustes 
NMDS.pro.r.df <- data.frame(rda1=pro.NMDS.r$Yrot[,1], rda2=pro.NMDS.r$Yrot[,2], xrda1 = pro.NMDS.r$X[,1], xrda2 = pro.NMDS.r$X[,2])
NMDS.pro.r.df$copdcontrol <- copdcaco.df.noblanks.no19586$COPD.control.blank
# Create the ggplot 
NMDS.pro.plot.r <- ggplot(NMDS.pro.r.df) +
geom_point(aes(x=rda1, y=rda2, colour=copdcontrol)) +
geom_point(aes(x=xrda1, y=xrda2, colour=copdcontrol)) +
geom_segment(aes(x=rda1,y=rda2,xend=xrda1,yend=xrda2,colour=copdcontrol),arrow=arrow(length=unit(0.2,"cm")))
NMDS.pro.plot.r + labs(title = "NMDS procrustes - randomised microbiome", y="NMDS second axis", x = "NMDS first axis", subtitle = "Correlation: 0.15, Significance: 0.38")
...
PCoA.pro.df$copdcontrol <- copdcaco.df.noblanks.no19586$COPD.control.blank
# Create the ggplot 
PCoA.pro.plot <- ggplot(PCoA.pro.df) +
geom_point(aes(x=rda1, y=rda2, colour=copdcontrol)) +
geom_point(aes(x=xrda1, y=xrda2, colour=copdcontrol)) +
geom_segment(aes(x=rda1,y=rda2,xend=xrda1,yend=xrda2,colour=copdcontrol),arrow=arrow(length=unit(0.2,"cm")))
PCoA.pro.plot + labs(title = "PCoA procrustes", y="PCo2", x = "PCo1", subtitle = "Correlation: 0.35, Significance: 6e-04")
...
```{r}
# Having spoken to Alex about my attempts with the co-occurrence network, he shared his work on some co-occurrence networks for the veal EFFORT data and shared code from this 
# As well as investigating bacteria-bacteria and ARG-ARG co-occurrences, I will most interestingly look at bacteria-ARG co-occurrences. 
...
write.graph(network_bact_bact, "network_bact_bact_graphml_file.graphml", format= "graphml") # Now import this file into gephi for graph visualisation and creation of networks here
...
# Export graph as graphml 
write.graph(network_ARG_ARG, "network_ARG_ARG_graphml_file.graphml", format= "graphml") # Now import this file into gephi for graph visualisation and creation of networks here
...
# Export graph as graphml 
write.graph(network_bacteria_ARG, "network_bacteria_ARG_graphml_file.graphml", format= "graphml") # Now import this file into gephi for graph visualisation and creation of networks here
...
```{r}
# nodes_1_bacteria <- data.frame(id = bact_bact_co_occurrence[,0], # id is the first column of the Spearman correlation table 
#                     label = bact_bact_co_occurrence$taxa.X, # the nodes will be labelled with the bacteria names from the correlation table 
#                     color = "blue") 
# 
# nodes_2_bacteria <- data.frame(id = bact_bact_co_occurrence[,0],
#                     label = bact_bact_co_occurrence$taxa.Y,
#                     color = "blue")
# 

# head(nodes_1_bacteria)
# head(nodes_2_bacteria)
# head(edges_bac_bac)
# 
# 
# nodes_bacteria_df <- data.frame(c(bact_bact_co_occurrence[,1],bact_bact_co_occurrence[,2]), color = "blue")

# Trying another method before using igraph
# nodes_1 <-
#   create_node_df(
#     n= 19,
#     type= NULL, 
#     label = bact_bact_co_occurrence$taxa.X,
#     style = "filled",
#     color = "aqua",
#     shape = "circle")
# 
...
# avector <- as.vector(bact_bact_co_occurrence$taxa.X)
# bvector <- as.vector(bact_bact_co_occurrence$taxa.Y)
# 
# edges_2 <-
#   create_edge_df(
#     from = as.factor(avector) ,
#     to = as.factor(bvector),
#     color = "red")
# 
# edges_1 <-
#   create_edge_df(
#     from = c("Atopobium" ,"Atopobium" ,"Olsenella","F0058", "Porphyromonas", "Prevotella","Rikenellaceae RC9 gut group", "Granulicatella","Granulicatella","Granulicatella","Mycoplasma","Gemella","Gemella","Lachnoanaerobaculum","Shuttleworthia", "[Eubacterium] yurii group","Megasphaera","Streptobacillus","Neisseria"),
#     to = c( "Megasphaera","Veillonella","Rikenellaceae RC9 gut group", "Rikenellaceae RC9 gut group","Gemella","Megasphaera","Shuttleworthia","Gemella","Neisseria","Haemophilus","[Eubacterium] brachy group","Neisseria","Haemophilus","Stomatobaculum","Anaeroglobus","Peptoanaerobacter","Veillonella","Alysiella","Haemophilus"),
#     color = "red")

# # Create a dataframe with node1, node2 and edge
# as.data.frame()
# nodes_bacteria_df_2 <- data.frame(c(bact_bact_co_occurrence[,1],bact_bact_co_occurrence[,2],bact_bact_co_occurrence[,3] ), color = "blue")
```
